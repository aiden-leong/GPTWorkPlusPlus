...
**Goal:** Replace the Python backend with a Rust core, a no-window silent launcher, and a Tauri management tool while preserving the existing GPT Work App injection behavior.

**Architecture:** Add a Rust workspace beside the current Python package, port behavior in verifiable slices, then switch the installed entry points to Rust and remove Python only after parity is proven. The only user-facing entry points are `GPT Work++` silent launch and `GPT Work++ 管理工具`; there is no separate CLI.
...
- Create `apps/codex-plus-launcher/`: no-window silent launcher binary used by the `GPT Work++` shortcut.
- Create `apps/codex-plus-manager/`: Tauri management console used by `GPT Work++ 管理工具`.
...
async fn main() -> Result<()> {
    println!("GPT Work++ launcher {}", codex_plus_core::version::VERSION);
    Ok(())
}
```
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("failed to run GPT Work++ manager");
}
```
...
Test Windows packaged GPT Work path detection, macOS `.app` executable construction, debug argument construction, and loopback port fallback.
...
- Shut down helper resources when GPT Work exits.
...
- Settings: provider sync, GPT Work command wrapper settings, user scripts summary.
...
```text
/Applications/GPT Work++.app
/Applications/GPT Work++ 管理工具.app
```
...
```text
双击 GPT Work++：静默启动增强版 GPT Work。
双击 GPT Work++ 管理工具：安装、卸载、更新、设置、日志和诊断。
```
...
- `GPT Work++` shortcut starts GPT Work without opening management UI.
- GPT Work App shows the GPT Work++ injected menu.
- Delete/undo/export/move/settings routes work from the injected UI.
- `GPT Work++ 管理工具` opens and shows Overview status.
