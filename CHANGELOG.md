...
- 修复启动 GPT Work 时会自动应用当前供应商配置的问题；现在只有手动点击“使用/切换供应商”才会切换供应商配置。
- 保留已开启的自动会话同步、插件市场配置修复、Computer Use guard 和历史模型名清理启动流程。

## 1.2.21 - 2026-06-28

- GPT Work 增强新增「插件列表全量展示」开关，进入插件页后自动连续展开「更多」入口。
- 自动展开支持「查看 ... 以及另外 N 个」和英文「View/Show ... and N more」按钮文案，减少插件市场分批展示时的重复点击。
- 自动展开默认开启，可在 GPT Work 增强页独立关闭；关闭后会停止后续自动展开任务。
...
- 模型列表改为左右双输入框：左侧填模型名，右侧填上下文窗口（如 `1M`、`200K` 或 `1000000`），右侧留空则使用 GPT Work 默认长度。
- 存储层新增 `model_windows` JSON map，与 `model_list` 彻底分离；GPT Work 客户端只使用无后缀模型名，避免模型选择器出现带后缀的历史项。
...
- 新增 Zed 远程项目记录能力，支持维护 GPT Work++ 可识别的远程项目最近列表，并为远程工作区打开提供更稳定的回退策略。
- 修复供应商同步在存在多条 `session_meta` 记录时只处理部分会话元数据的问题。
- 修复 Windows 单实例启动保护，在默认端口被异常占用时改用更稳健的锁与端口回退逻辑，降低无法启动的概率。
- 限制 GPT Work 快速服务档位只对支持的模型生效，避免不兼容模型收到无效配置。
...
- 新增独立的「工具与插件」页面，用于统一管理 GPT Work++ / GPT Work 的 MCP、skills、plugins，不再绑定到单个供应商。
- 切换供应商时会合并当前启用的工具与插件配置，同时避免把供应商专属配置误写入通用配置。
- 工具与插件列表改为从当前 GPT Work 配置实时读取启用状态，支持直接开关和删除条目。
